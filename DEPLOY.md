...
## 9. DB 복구 런북 (2026-07-09 최초 검증 · 2026-07-24 재리허설 통과)

> Render 디스크 유실·데이터 사고 시 Drive 오프사이트 백업으로 복원하는 절차.
> **리허설 결과**: 전 절차 통과 — 무결성 ok·FK 위반 0·서버 기동 ~1초·
> 마이그레이션 멱등 통과·전 화면 200·쓰기 정상. 첨부는 전량 Drive 저장이라 DB만 복원하면 됨.
> **2026-07-24 재리허설**(격리 워크스페이스, 개발 DB 사본): cron과 동일한 `backupDatabase()`(VACUUM INTO)로 스냅샷 생성 → 무결성 ok·FK 0·건수 대조 일치 → 복원본으로 기동(마이그레이션 에러 0) → 주요 화면 11개 전부 200 → 프로젝트 생성·삭제 쓰기 정상 → 쓰기·WAL 반영 후 최종 무결성 ok. 07-09 이후 스키마 변경(equipment 테이블·지급 축 잠금 등)에도 절차 그대로 통과. admin_state의 마이그레이션 게이트(`*_v1`·`*_backfill*`)가 존재해 재기동 시 skip(멱등) 확인.
