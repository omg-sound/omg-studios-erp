...
/**
 * 요청별 테마 컨텍스트(2026-07-21) — 서버가 `<html>`에 data-theme를 **첫 페인트에** 렌더하기 위한 것.
 *
 * 왜: 테마(라이트/다크)는 그동안 theme-init.js가 <head>에서 세팅했는데, dev는 그 파일을 매 이동마다
 ...
 * 테스트는 컨텍스트 없이 layout()을 직접 부르므로 기본값(테마 없음 = OS 추종)을 돌려준다.
 *
 * theme-init.js는 여전히 남는다 — 첫 방문(쿠키 없음)·OS 테마 변경·쿠키만 지운 경우의 폴백 + 쿠키 기록(다음 이동부터 서버가 앎).
 *
 * ⚠️ 팔레트(시각 스타일 선택) 축은 2026-07-30 폐지 — 테마가 하나(라이트/다크만)라 흘릴 값이 theme 하나다.
 *    브라우저에 남은 옛 `palette` 쿠키는 아무도 읽지 않아 무해하다(만료되면 사라진다).
 */
...
/**
 * `<html>`에 넣을 속성값. 쿠키가 없으면 테마 미설정(= OS 추종을 CSS/theme-init에 위임).
 */
