...
const { runWithTheme } = require("./lib/request-theme");
...
app.use("/invoices", invoiceRoutes); // 목록·생성·삭제·PDF=requireBilling(치프·대표·스태프), 계산서·입금 처리=requireInvoice(치프·대표)
...
app.use(
  express.static(path.join(__dirname, "..", "public"), {
    index: false,
    // 1시간 → 1년 immutable(2026-07-20 성능 점검). 참조 URL이 전부 `?v=<ASSET_VERSION>`이라
    // 파일이 바뀌면 URL 자체가 바뀐다 → 옛 캐시를 붙들 수 없다. 1시간이면 한 시간마다 자산 4개를
    // 재검증(304)하는데, 원본이 싱가포르라 왕복 1회가 ~65ms다. immutable은 그 재검증 자체를 없앤다.
    // ⚠️ 전제: layout이 참조하는 정적 파일이 **전부** views.js ASSET_FILES에 들어 있어야 한다.
...
// 배포 재시작(SIGTERM) 시 진행 중인 알림 전송(fire-and-forget)을 최대 5초 기다린 뒤 종료.
// main 커밋=자동배포라 재시작이 잦은데, 즉시 종료되면 방금 발행한 청구의 알림이 무음 유실된다(2026-07-13 점검).
