# OMG Studios ERP — 작업 이어가기 가이드

> 녹음/믹싱 스튜디오 **운영–정산 통합 ERP**(예약 → 작업 → 청구 → 정산). 사내 시스템(웹 기반).
> **표준 호칭 = "ERP"**(2026-07-17 — 사용자가 'ERP'라 하면 이 제품). **제품 정체성 = 녹음 스튜디오 전용 버티컬 미니 ERP**(order-to-cash 흐름을 단일 데이터 모델로 통합). 호칭 규칙·스코프 기준·경계는 [`CLAUDE.md` 📛 표준 호칭 · 🎯 제품 정체성](./CLAUDE.md) 참조 — 신규 기능은 "이 스튜디오의 운영–정산 흐름에 속하나?"로 판단.
...
### 6-0. 테스트 체계 — 3층 방어선 + 스모크 (`npm test`, 304개, CI Node 20/22 동일 실행)
...
- UI(`guardrails-ui.test.js`): ⑧**data-\* 마커 계약**(app.js가 찾는 모든 마커는 어딘가에서 렌더돼야 — 서버/JS 어느 쪽 리네임 드리프트든 실패. 도입 첫 실행에서 죽은 참조 5건 검출·제거) ⑨콤보 보이는 입력(data-pc/cc/pk-input) name 금지 ⑩CSP 계약(인라인 핸들러·`<script>` 금지 — 배포에서만 조용히 죽는 드리프트) ⑪IME 가드 강제(Enter/방향키 keydown엔 isComposing 필수) ⑫금액칸↔MONEY 정규식 계약 ⑬personCombo 옵션 JSON 키↔app.js 소비 정합 ⑭companyCombo 사람 옵션 키(kind/alt/honorific)↔app.js dispOf·자동채움 정합 ⑮시간 콤보 팝업은 scrollTop으로 정렬(scrollIntoView 금지 — 페이지 튐 방지)
...
```bash
npm test                                   # 전체 304개(단위+가드+상호작용+스모크)
...
8. **타이포그래피 및 Elevation 스케일 정규화 (코드 품질)** — `DESIGN.md` "Known Gaps" 해소. 텍스트 스케일(`text-body`, `text-heading-*` 등)과 그림자 뎁스 토큰(`shadow-elevation-*` 등)을 `tailwind.config.js`와 `src.css`에 정의하고, 뷰 파일(`src/views*.js`) 곳곳의 유틸리티 클래스 전수 교체.
9. (보류) **content_type/billing_type UI 노출** — `content_type[Music|Video_Post]`·`billing_type` 현재 미노출/강제; 영상 구분·과금 유형 선택은 향후 확장 시 복원.

> **완료(2026-07-19 후속)**: **매출 기간 렌즈 분리(subagent-driven 7태스크)** — 마스터-디테일 도입 직후 넣은 아래 '기간 변경 시 선택 유지'(`revPeriodControl({sel})`)가 몇 시간 만에 도로 빠졌다: 기간(언제)과 순위(누구)를 한 화면에 섞으니 헷갈려 **기간 컨트롤을 개요 탭 전용으로 이동**, 스탭별·업체개인별은 **기간 없이 전체 누적**으로 전환(`revenueByStaff/ByPayer`가 period 인자 없이 호출). 목록 행에 **최근 거래월**(`최근 2026.7`, `lastSeenLabel`)+기간 없는 링크. 상세(`revStaffDetail`/`revPayerDetail`)는 **월별 그룹+월 소계**(`groupByMonth`/`monthHeader`, 최신 월 우선) — **스탭 상세는 한 월 안에서 작업·세션을 날짜순으로 섞는다**(종류는 배지만, 사용자 확정: 월 리듬이 목적이라 종류로 먼저 가르면 두 번 쪼개짐). `issuedInPeriodSql`에 **전체 기간 모드**(`year` 없음/`'all'` → 무조건). `revenueForStaff` 세션 행에 `payout` 파생(월 소계 순이익). 개요 Top5 아래는 **'전체 N명/N곳 보기' `<details>` 펼침**(옛 '전체 보기' 링크 제거 — 목록이 누적 전용이 되며 그 링크가 다른 기간으로 보내는 거짓말이 됐다). 구 드릴다운 `/revenue/staff|payer/:id` 302 리다이렉트는 유지. 설계·계획=`docs/superpowers/{specs,plans}/2026-07-19-revenue-lens-split*`.
> **완료(2026-07-19)**: **매출 스탭별·업체개인별 탭 = 마스터-디테일**(브레인스토밍→설계문서→계획→subagent-driven 6태스크) — 왼쪽 순위 목록 + 오른쪽 상세 패널로 전환. **골격은 `contactPanes` 재사용**(연락처·업체그룹과 동일), **선택은 URL 쿼리만**(`?tab=staff&staff=<id>`) → 신규 JS 0·데이터 레이어 무변경. **세 탭 모두 wide 복귀**(직전 698c596의 읽기 폭 결정을 대체 — 패널 오른쪽이 `max-w-content`라 읽기 폭은 보장). **패널 높이 `REV_PANE_H`=`lg:h-[calc(100vh-15.5rem)]` 실측 확정**(1710px: 상단 222px+하단 24px → 669px 이하 필요, 15rem은 6px 넘쳐 페이지가 스크롤됐다). **목록 폭 키 `revListW` 분리**(`contactPanes({widthKey,heightClass})` 파라미터·기본값은 연락처 기준이라 기존 두 화면 무변경, app.js가 `data-cl-width-key`에서 읽음). **기간 변경 시 선택 유지**(`revPeriodControl({sel})` hidden — 2026-07-19 후속 '기간 렌즈 분리'에서 제거됨, 위 항목 참조). **상세 경로 단일화** — 구 `/revenue/staff|payer/:id`는 302, 개요 Top5도 패널 URL, `revStaffTable`/`revPayerTable` 삭제('상세가 두 벌' 재발 방지). 435 테스트·브라우저 실측(스크롤 0·폭키 분리·900/390 반응형·구 URL 302). 설계·계획=`docs/superpowers/{specs,plans}/2026-07-19-revenue-master-detail*`.
> **완료(2026-07-15)**: **2차 전수 점검(워크플로 4렌즈)** — 확정 12건 수정: 업체 재사용 분기의 담당자 해제·공동대표 교체(파괴적 2건, smoke 잠금)·날짜 콤보 제출 경합(동기 flush+required 복원)·무변경 autosubmit 리로드·추천 칩 폴백 자기파괴·confirm 취소 8초 잠김·회사명 정규화 SQL/JS 일원화·청구 목록 limit 보존·N+1 배치화·부제 병기 중복·내일/모레 기준일·고아 정리. 291 테스트.
> **완료(2026-07-15)**: **청구 목록 지메일식 한 줄 행 + 밀도 토글** — 좌측=청구처·(제작사·아티스트) / 우측=금액+상태 pill 2개. 행=펼침(청구 항목·PDF)·청구처=상세. pill 클릭이 펼침을 토글하지 않게 preventDefault→requestSubmit. 밀도는 프로젝트 목록과 같은 설정 공유. 282 테스트.
...
> **완료(2026-07-11h)**: **완료 UI 일관성 정리**(개선점 찾기에서 선택) — 네 완료 표면 중 캘린더 팝오버만 '완료 처리'/'✓ 완료됨'(btn-primary)로 달랐던 것을 나머지와 동일한 `− 완료`/`✓ 완료` glyph 토글로 통일(모달이라 btn-sm 크기만). `sessionCardModal` 버튼만. 199 테스트·실브라우저 확인.
> **완료(2026-07-11g)**: **프로젝트 목록 카드 펼침에서 바로 세션 완료**(사용자 요청 — 목록 펼침 미리보기엔 완료가 없어 프로젝트 상세·일정으로 들어가던 2중3중) — `projectSummaryHtml` 펼침 세션에 완료 토글, `POST /sessions/:id/status`+`return=/projects?open=projectId` → 완료 후 그 카드 재펼침(`<details open>`)·스크롤 보존, 일반 리로드는 접힘. `listProjectSummaries` 세션 id 추가. `src/data/projects.js`·`src/views.projects.js`·`src/routes/projects.routes.js`. 199 테스트·E2E·실브라우저 검증.
