...
// 2026-07-20 사용자 리포트 '연락처에서 처음 선택할 때만 스크롤이 바뀐다(이후는 안 바뀐다)'.
// 원인: 키를 URL로 만들어 목록(`/contacts`, 탭 쿼리 없음)과 상세(`/contacts/24?tab=all`)가 서로 다른 키였다.
// → 서버가 [data-nav-list="contacts:all"]로 키를 찍고 app.js는 그 값을 쓴다(활성 탭의 기본값은 서버만 안다).
test("nav-list: 키는 서버가 찍어준 값을 쓴다(URL이 달라도 목록·상세가 같은 키)", () => {
  const html = `<div data-nav-list="contacts:all" style="overflow:auto">
      <a href="/contacts/24?tab=all">A</a>
      <a href="/contacts/25?tab=all" aria-current="true">B</a>
    </div>`;
  // 목록 URL엔 ?tab이 없다 — 옛 방식이면 여기서 'navScroll:contacts'로 저장돼 상세에서 못 찾았다.
  const list = mountDom(html, { url: "http://localhost/contacts" });
  list.doc.querySelector("[data-nav-list]").scrollTop = 620;
  list.doc.querySelectorAll("[data-nav-list] a")[0].click();
  assert.equal(list.win.sessionStorage.getItem("navScroll:contacts:all"), "620", "서버 키로 저장");
  assert.equal(list.win.sessionStorage.getItem("navScroll:contacts"), null, "URL 기반 옛 키로는 저장하지 않는다");

  // 상세 URL(?tab=all)에서 같은 키로 복원된다 — 첫 선택에서도 위치가 유지된다.
  const detail = mountDom(html, { url: "http://localhost/contacts/24?tab=all", session: { "navScroll:contacts:all": "620" } });
  assert.equal(detail.doc.querySelector("[data-nav-list]").scrollTop, 620);
});

test("nav-list: 서버 키가 없으면 옛 URL 방식으로 폴백(마커만 있는 목록 호환)", () => {
  const html = `<div data-nav-list style="overflow:auto"><a href="/x" aria-current="page">A</a></div>`;
  const { win, doc } = mountDom(html, { url: "http://localhost/revenue?tab=staff" });
  doc.querySelector("[data-nav-list]").scrollTop = 310;
  doc.querySelector("[data-nav-list] a").click();
  assert.equal(win.sessionStorage.getItem("navScroll:revenue:staff"), "310");
});

// ── 프로젝트 목록 헤더 클릭 정렬(2026-07-20 사용자 요청) ────────────────────────────────
// 마크업 계약은 project-list.test.js가 잠그고, 여기서는 **실제 app.js를 돌려 동작**을 본다:
// 방향 토글·타입별 비교·빈 값 뒤로·펼친 행 유지. 청구 목록과 같은 공용 코어(wireSortHeaders)를 쓴다.
const { projectTableHead, projectListRow } = require("../src/views.projects");

function mountProjectList(rows) {
  const emptySummary = { sessions: [], tracks: [], taskTypes: [] };
  const base = {
    contact_name: "", manager_name: "", next_session_date: null, sess_scheduled: 0, sess_done: 0,
    task_cnt: 0, task_pending: 0, task_done: 0, unbilled_cnt: 0, track_titles: "",
    task_total: 0, session_amount_total: 0, rate: 0, invoice_discount_total: 0,
  };
  const html = `<div data-sort-rows>${projectTableHead()}${rows
    .map((r, i) => projectListRow({ id: i + 1, ...base, ...r }, emptySummary, { tab: "active" }))
    .join("")}</div>`;
  return mountDom(html);
}

const sortVals = (doc, key) =>
  [...doc.querySelectorAll(`[data-sort-row] [data-sort-key="${key}"]`)].map((c) => c.getAttribute("data-sort-value"));

test("프로젝트 목록 정렬: 첫 클릭=오름차순, 같은 항목 다시=내림차순(aria-sort로 화살표)", () => {
  const { doc } = mountProjectList([
    { title: "나 프로젝트", client_name: "다회사" },
    { title: "가 프로젝트", client_name: "가회사" },
    { title: "다 프로젝트", client_name: "나회사" },
  ]);
  const th = doc.querySelector('.proj-thead [data-sort-key="company"]');
  th.click();
  assert.deepEqual(sortVals(doc, "company"), ["가회사", "나회사", "다회사"], "오름차순");
  assert.equal(th.getAttribute("aria-sort"), "ascending");
  th.click();
  assert.deepEqual(sortVals(doc, "company"), ["다회사", "나회사", "가회사"], "같은 항목 다시 = 내림차순");
  assert.equal(th.getAttribute("aria-sort"), "descending");
});

test("프로젝트 목록 정렬: 금액은 숫자 비교(문자열이면 ₩1,500,000 < ₩300,000 이 된다)", () => {
  const { doc } = mountProjectList([{ rate: 300000 }, { rate: 1500000 }, { rate: 800000 }]);
  doc.querySelector('.proj-thead [data-sort-key="amount"]').click();
  assert.deepEqual(sortVals(doc, "amount"), ["300000", "800000", "1500000"]);
});

test("프로젝트 목록 정렬: 값 없는 칸은 방향과 무관하게 뒤로", () => {
  const { doc } = mountProjectList([{ rate: 0 }, { rate: 500000 }, { rate: 100000 }]);
  const th = doc.querySelector('.proj-thead [data-sort-key="amount"]');
  th.click();
  assert.equal(sortVals(doc, "amount").at(-1), "", "오름차순에서 뒤");
  th.click();
  assert.equal(sortVals(doc, "amount").at(-1), "", "내림차순에서도 뒤(값 없는 건 '가장 작은 값'이 아니다)");
});

test("프로젝트 목록 정렬: 다른 항목을 누르면 이전 항목의 정렬 표시가 풀린다", () => {
  const { doc } = mountProjectList([{ client_name: "나" }, { client_name: "가" }]);
  const company = doc.querySelector('.proj-thead [data-sort-key="company"]');
  const title = doc.querySelector('.proj-thead [data-sort-key="title"]');
  company.click();
  title.click();
  assert.equal(company.getAttribute("aria-sort"), "none", "화살표가 두 열에 동시에 뜨면 안 된다");
  assert.equal(title.getAttribute("aria-sort"), "ascending");
});

test("프로젝트 목록 정렬: 펼쳐둔 행은 정렬 후에도 펼쳐진 채(행 노드를 옮길 뿐)", () => {
  const { doc } = mountProjectList([{ client_name: "나" }, { client_name: "가" }]);
  const row = doc.querySelector("#proj-1");
  row.open = true;
  doc.querySelector('.proj-thead [data-sort-key="company"]').click();
  assert.equal(doc.querySelector("#proj-1").open, true);
  assert.equal(doc.querySelectorAll("[data-sort-row]").length, 2, "행이 유실·복제되지 않는다");
});

test("프로젝트 목록 정렬: 키보드(Enter)로도 정렬, IME 조합 중 Enter는 무시(함정 #18)", () => {
  const { win, doc } = mountProjectList([{ client_name: "나" }, { client_name: "가" }]);
  const th = doc.querySelector('.proj-thead [data-sort-key="company"]');
  fire(win, th, "keydown", { key: "Enter", isComposing: true });
  assert.equal(th.getAttribute("aria-sort"), "none", "조합 중 Enter는 무시");
  fire(win, th, "keydown", { key: "Enter" });
  assert.equal(th.getAttribute("aria-sort"), "ascending");
...
// ── 제안 칩([data-fill-value][data-fill-target]) 클릭 → 대상 입력칸 채움(장비 종류·장소) ──
test("제안 칩 클릭: 대상 입력칸에 값을 채우고 change를 발화한다(dirty 감시 통지, 함정 #23)", () => {
  const html = `<form>
    <input name="location" data-equip-location />
    <button type="button" data-fill-value="창고" data-fill-target="[data-equip-location]">창고</button>
  </form>`;
  const { win, doc } = mountDom(html);
  const input = doc.querySelector("[data-equip-location]");
  const chip = doc.querySelector("[data-fill-value]");
