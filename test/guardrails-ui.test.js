...
// ⑰ 정적 자산 캐시 = 1년 immutable이므로, **참조하는 파일이 전부 ASSET_VERSION 계산에 들어가야** 한다.
// 안 들어간 파일은 내용이 바뀌어도 URL(?v=)이 그대로라 브라우저가 **1년 동안 옛 파일을 쓴다**
// (캐시가 1시간이던 시절엔 곧 만료돼 안 보이던 종류의 결함이 2026-07-20 캐시 연장으로 치명적이 됐다).
test("정적 자산: layout이 참조하는 css/js가 전부 ASSET_FILES에 포함된다", () => {
  const fs = require("fs");
  const path = require("path");
  const src = fs.readFileSync(path.join(__dirname, "..", "src", "views.js"), "utf8");
  const listed = (src.match(/const ASSET_FILES = \[([^\]]*)\]/) || [])[1] || "";
  const files = [...listed.matchAll(/"([^"]+)"/g)].map((m) => m[1]);
  assert.ok(files.length >= 4, "ASSET_FILES 목록을 찾지 못했다");

  // 실제로 HTML에 내보내는 참조를 전부 긁어 대조한다(?v= 유무와 무관하게 경로만 본다).
  const referenced = [...src.matchAll(/"\/(css|js)\/([A-Za-z0-9._-]+)\?v=/g)].map((m) => `${m[1]}/${m[2]}`);
  assert.ok(referenced.length >= 4, "정적 참조를 찾지 못했다");
  for (const ref of new Set(referenced)) {
    assert.ok(files.includes(ref), `${ref}가 ASSET_FILES에 없다 — 이 파일만 고치면 캐시가 1년 동안 안 깨진다`);
  }

  // 참조는 예외 없이 버전 쿼리를 달아야 한다(맨몸 경로가 있으면 그 파일이 1년 고정된다).
  const bare = [...src.matchAll(/"(\/(?:css|js)\/[A-Za-z0-9._-]+)"/g)].map((m) => m[1]);
  assert.deepEqual(bare, [], `버전 없는 정적 참조: ${bare.join(", ")}`);
});
