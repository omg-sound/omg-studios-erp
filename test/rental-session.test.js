...
// 대관 세션 판정 회귀 테스트 — 2026-07-04 추가, 2026-07-30 종류명 촬영 → 촬영 대관/작업 대관 분리.
...
test("config: 대관 종류·kind 매핑 정합", () => {
  assert.deepEqual(RENTAL_SESSION_TYPES, ["녹음", "촬영 대관", "작업 대관", "공연"], "시간제 대관 매출 = 녹음·촬영 대관·작업 대관·공연");
  ...
  // 카테고리 → kind 분기: 녹음=recording·촬영 대관=filming·작업 대관=workspace·공연=performance.
...
test("sessionRateAmount: 대관 계열은 산정, 그 외 종류는 null", () => {
...
test("sessionRateAmount: 단가 미선택·시간 없음은 null(결핍 사유 안내 대상)", () => {
  assert.equal(sessionRateAmount({ session_type: "촬영 대관", rate_item_id: null, start_time: "10:00", end_time: "14:00" }), null);
  assert.equal(sessionRateAmount({ session_type: "촬영 대관", rate_item_id: rateId, start_time: null, end_time: null }), null);
});
...
// ── 세션 외주 정산(2026-07-06 사용자 상담 — track_tasks와 동일 구조로 세션도 지급단가·지급상태 관리) ──
test("세션 담당 엔지니어 정산: 지급단가 저장 + 수정 시 계속 배정된 엔지니어의 지급상태 보존, 지급 처리 토글", () => {
  const D = require("../src/data");
  const CHIEF = { id: 1, role: "chief", email: "chief@omg.test" };
  const worker = Number(db().prepare("INSERT INTO project_managers (name, active) VALUES ('외주엔지A', 1)").run().lastInsertRowid);
  const projId = Number(db().prepare("INSERT INTO projects (title, project_type, rate) VALUES ('세션정산테스트','session',0)").run().lastInsertRowid);
  const s = D.createSession(CHIEF, projId, {
    session_type: "믹싱", session_date: "2026-08-04", all_day: "1", status: "예정",
    engineer_ids: [String(worker)], engineer_rates: ["150000"],
  });
  let eng = D.listSessionEngineers(s.id)[0];
  assert.strictEqual(eng.worker_rate, 150000, "지급단가 저장");
  assert.strictEqual(eng.worker_paid, 0);

  D.setSessionEngineerPayout(s.id, worker, true);
  eng = D.listSessionEngineers(s.id)[0];
  assert.strictEqual(eng.worker_paid, 1, "지급 처리");
  assert.ok(eng.worker_paid_date, "지급일 기록");

  // 같은 엔지니어를 유지한 채 단가만 수정 — 지급 처리 상태는 보존돼야 한다(2026-07-06 버그수정 대상).
  D.updateSession(CHIEF, s.id, {
    session_type: "믹싱", session_date: "2026-08-04", all_day: "1", status: "예정",
    engineer_ids: [String(worker)], engineer_rates: ["200000"],
  });
  eng = D.listSessionEngineers(s.id)[0];
  assert.strictEqual(eng.worker_rate, 200000, "단가 갱신 반영");
  assert.strictEqual(eng.worker_paid, 1, "계속 배정된 엔지니어의 지급 처리 상태는 보존");

  // 새 엔지니어 추가 — 새로 배정된 쪽은 지급 미처리(0)로 시작.
  const worker2 = Number(db().prepare("INSERT INTO project_managers (name, active) VALUES ('외주엔지B', 1)").run().lastInsertRowid);
  D.updateSession(CHIEF, s.id, {
    session_type: "믹싱", session_date: "2026-08-04", all_day: "1", status: "예정",
    engineer_ids: [String(worker), String(worker2)], engineer_rates: ["200000", "80000"],
  });
  const engs = D.listSessionEngineers(s.id);
  const e1 = engs.find((e) => e.id === worker), e2 = engs.find((e) => e.id === worker2);
  assert.strictEqual(e1.worker_paid, 1, "기존 엔지니어 지급 상태 계속 보존");
  assert.strictEqual(e2.worker_rate, 80000, "새 엔지니어 단가 저장");
  assert.strictEqual(e2.worker_paid, 0, "새 엔지니어는 미지급으로 시작");

  const payouts = D.listSessionPayoutsForWorker({ id: worker, name: "외주엔지A" });
  assert.strictEqual(payouts.length, 1);
  assert.strictEqual(payouts[0].worker_rate, 200000);
  assert.strictEqual(payouts[0].worker_paid, 1);
});
