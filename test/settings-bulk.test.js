...
test("bulkUpdateRateItems: 여러 행 동시 갱신·미참여 행 불변·임의 id 무시·이름 비운 행만 건너뜀", () => {
  ...
  const r = D.bulkUpdateRateItems({
    [`rate_name_${a.id}`]: "벌크A2", [`category_${a.id}`]: "스튜디오 녹음", [`base_hours_${a.id}`]: "5", [`base_price_${a.id}`]: "500,000", [`extra_hours_${a.id}`]: "1", [`extra_price_${a.id}`]: "100000",
    [`rate_name_${b.id}`]: "", [`category_${b.id}`]: "스튜디오 녹음", [`base_hours_${b.id}`]: "4", [`base_price_${b.id}`]: "400000", [`extra_hours_${b.id}`]: "1", [`extra_price_${b.id}`]: "100000",
...
test("bulkUpdateTaskTypes: 갱신 + sort_order 보존(updateTaskType의 100 리셋 잠복 버그 방어)", () => {
  ...
  D.bulkUpdateTaskTypes({
    [`label_${t1.id}`]: "벌크작업1-개명", [`billing_type_${t1.id}`]: "Fixed_Per_Track", [`unit_price_${t1.id}`]: "150,000",
    // is_quick 미전송 = 해제
  });
