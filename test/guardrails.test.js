...
// ⑩ 테스트의 DB 격리(2026-07-20 — CI가 69번 연속 적색이던 진짜 원인).
// 사고 이력: `revenue-views`·`clients-views`가 DB_PATH를 안 잡은 채 렌더 함수를 호출했다. 그 함수들이
// 지연 조회로 DB를 건드리는데(revStaffDetail→taskTypeLabel→task_types), 개발 머신에선 `./data/app.db`를
// 조용히 써서 통과하고 그 파일이 없는 CI에서만 'no such table'로 죽었다 — **로컬 실행으로는 안 보인다**.
// 방어는 파일 스캔이 아니라 **DB를 여는 순간의 런타임 가드**로 둔다(src/db.js): 순수 로직 테스트는
// DB를 안 열어 걸리지 않고, 실제로 여는 테스트만 즉시·명확히 실패한다.
test("guardrail: 테스트가 DB_PATH 없이 DB를 열면 즉시 실패한다(개발 DB 유출·CI 적색 방지)", () => {
  const dbSrc = read(path.join("src", "db.js"));
  assert.match(dbSrc, /if \(process\.env\.NODE_TEST_CONTEXT\) \{/, "db()에 테스트 격리 가드가 있어야");
  // '경로가 비었나'가 아니라 '저장소 안 DB인가'로 판정해야 한다 — .env가 DB_PATH=./data/app.db를 넣어주므로
  // 빈 값 검사로는 절대 안 걸린다(이번 사고의 실제 메커니즘).
