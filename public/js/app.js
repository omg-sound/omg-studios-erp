...
// ── 날짜 콤보([data-date-combo]) — 앱 전 날짜 칸 공용(2026-07-14 세션 폼 → 전역 승격) ──
// 서버는 views.dateCombo가 hidden(값·name)+보이는 칸+빈 팝오버만 렌더하고, 월 그리드·타이핑 파싱은 여기서.
(function () {
  "use strict";
  // YYYY-MM-DD에 일수 더하기(로컬 파트 조립 — toISOString은 UTC라 KST에서 하루 밀림).
  function addDays(v, days) {
    var t = new Date(String(v) + "T00:00:00");
    if (isNaN(t)) return v || "";
  ...
  function initDateCombos(root) {
    Array.prototype.forEach.call((root || document).querySelectorAll("[data-date-combo]"), function (wrap) {
      if (wrap.__dcInit) return; // 중복 초기화 방지(동적 삽입 대비)
      wrap.__dcInit = true;
  ...
  window.__initDateCombos = initDateCombos; // 동적으로 삽입된 폼(행 복제 등)에서 호출
})();
...
(function () {
  ...
  Array.prototype.forEach.call(form.querySelectorAll("[data-line-input]"), function (inp) {
    ...
    // (옛 최소가 하한 되올림은 2026-07-29 가격 유형 폐기와 함께 제거 — 금액은 언제나 자유롭게 고친다.)
